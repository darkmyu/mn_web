import { useProfileControllerFollowingsSuspenseInfinite } from '@/api/profile';
...
function ProfileFollowingList({ username }: Props) {
  const { ref, inView } = useInView();

  const { data, hasNextPage, isFetched, isFetchingNextPage, fetchNextPage } =
    useProfileControllerFollowingsSuspenseInfinite(
      username,
      {
        limit: 50,
      },
  ...
  const followings = data.pages.flatMap((page) => page.data.items);
  ...
  return (
    <div className="flex flex-col gap-8">
      <p className="text-base font-medium lg:text-lg">{`${formatNumber(total)}명을 팔로우 중`}</p>
